...
# Load GPT-OSS-120B (requires 80GB GPU due to MXFP4 quantization)
model_path = "openai/gpt-oss-120b"
model = AutoModelForCausalLM.from_pretrained(
    model_path,
    torch_dtype="auto",  # Model ships with optimized quantization
    device_map="auto",
...
GPT-OSS ships with MXFP4 quantization that compresses the MoE weights to 4 bits, enabling the entire 117B parameter model to fit within a single 80GB GPU. This represents a breakthrough in accessibility—what previously required massive multi-GPU clusters is now deployable on standard enterprise hardware. For production deployment, institutions can leverage expert parallelism to distribute the 128 experts across multiple GPUs for even faster inference during high-frequency trading scenarios.
